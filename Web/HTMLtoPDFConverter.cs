...
namespace Web
{
    public class HTMLtoPDFConverter
    {
        ...
        public static bool HtmlToPdf(string url, string path)
        {
            try
            {
                ...
                using (Process p = new Process())
                {
                    ...
                    //string folder = @"C:\soft\wkhtmltopdf\";
                    //folder = @"E:\wkhtmltopdf\";
                    string folder = new System.IO.DirectoryInfo(AppDomain.CurrentDomain.BaseDirectory).Parent.FullName + @"\wkhtmltopdf\";
                    string str = folder + "wkhtmltopdf.exe";
                    if (!File.Exists(str))
                    {
                        return false;
                    }
        ...
        private static void KillWKHtmltoPDF()
        {
            ...
            {
                Process[] procs = Process.GetProcessesByName("wkhtmltopdf");
                Array.ForEach(procs, delegate(Process proc) { proc.Kill(); });
            }
            catch (Exception ex) { }
        }
    }
